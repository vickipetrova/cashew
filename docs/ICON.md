...
Everything about how Cashew's icon is authored, rendered and packaged. `CLAUDE.md` keeps the
...
- **Tier 1, always.** `sips` and `iconutil` — both in `/usr/bin` — turn `assets/icon-1024.png` into
  `Contents/Resources/Cashew.icns`, and the plist gets `CFBundleIconFile`. No Xcode.
- **Tier 2, when `xcrun --find actool` succeeds.** `actool` compiles `assets/Cashew.icon` into
...
`assets/Cashew.icon` is the source of truth, and **`assets/icon-1024.png` is a committed render of
...
- **`actool` exits 0 when it renders nothing, so `Assets.car` is not the signal.** If `--app-icon`
  names an asset the document doesn't contain — rename `Cashew.icon`, or change `APP_NAME`, and it
...
- **`actool` must compile to a staging directory.** It also writes its own `Cashew.icns`, so
...
`NSWorkspace.shared.icon(for: .application)` instead of null-checking. `open build/Cashew.app` is the
