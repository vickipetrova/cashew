...
/// runs its bundled `Contents/Helpers/cashew-hook`. Every other key, and every other tool's hook, is carried through.
///
...
/// so a deleted Cashew's leftover hooks exit 0 and say nothing.
struct HookInstaller {
    /// What makes a hook Cashew's. The bundle path, not just the helper's name, so a user's own
    ...
    /// and the session shows a "hook error" notice on every event, so a deleted or moved Cashew
