...
@Suite struct HookInstallerTests {
    ...
    @Test func sharedEntryKeepsTheOtherCommand() throws {
        let settings = try object("""
            {"hooks": {"PreToolUse": [{"matcher": "*", "hooks": [
              ...
              {"type": "command", "command": "'/Users/v/Old/Cashew.app/Contents/Helpers/cashew-hook' pre"}]}]}}
            """)
        #expect(commands(HookInstaller.merged(settings, helperPath: nil), "PreToolUse") == ["other-tool pre"])
    }

    /// Only the bundled helper is Cashew's. A user's own script that happens to share the name is
    /// theirs, and survives both install and removal.
    @Test func aLookalikeUserHookIsLeftAlone() throws {
        let settings = try object("""
            {"hooks": {"Stop": [{"hooks": [{"type": "command", "command": "~/bin/my-cashew-hook-script.sh"}]}]}}
            """)
        #expect(commands(HookInstaller.merged(settings, helperPath: nil), "Stop") == ["~/bin/my-cashew-hook-script.sh"])
        #expect(commands(HookInstaller.merged(settings, helperPath: helper), "Stop")
                == ["~/bin/my-cashew-hook-script.sh", HookInstaller.command(helperPath: helper, event: .stop)])
    }
    ...
    @Test func pathsAreShellQuoted() {
        #expect(HookInstaller.command(helperPath: "/Users/o'neil/Cashew.app/Contents/Helpers/cashew-hook",
                                      event: .stop)
                == #"[ -x '/Users/o'\''neil/Cashew.app/Contents/Helpers/cashew-hook' ] || exit 0; "#
                    + #"exec '/Users/o'\''neil/Cashew.app/Contents/Helpers/cashew-hook' stop"#)
    }

    /// Claude Code shows a hook error notice for a command that isn't there, so a deleted or moved
    /// Cashew must make its leftover hooks exit 0 quietly. Runs the real shell, never ~/.claude.
    @Test func missingHelperIsAQuietNoOp() throws {
        let process = Process()
        process.executableURL = URL(fileURLWithPath: "/bin/sh")
        let missing = FileManager.default.temporaryDirectory
            .appendingPathComponent("cashew-missing-\(UUID().uuidString)/it's gone/cashew-hook").path
    ...
    @Test func onlyApplicationsFoldersAreRunnable() {
        let home = "/Users/x"
        let suffix = "Cashew.app/Contents/Helpers/cashew-hook"
        ...
        let dev = "/Users/x/dev/cashew/build/\(suffix)"
        ...
        #expect(!HookInstaller.isRunnableLocation("/Volumes/Cashew/\(suffix)", home: home, allowAnywhere: false))
    ...
    private struct Sandbox {
        ...
        var helper: URL { root.appendingPathComponent("Cashew.app/Contents/Helpers/cashew-hook") }
